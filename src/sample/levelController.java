...
public class levelController {
    ...
    private void openSelectedGameMode(Modes selectedMode) {
        try{
            ...
            switch (selectedMode){
                case Accuracy:
                    Mode m1 = new Mode(Modes.Accuracy, c, context);
                    m1.setCurrentUser(currentUser);
                    controller.setMode(m1);
                    break;
                case Speed:
                    Mode m2 = new Mode(Modes.Speed, c, context);
                    m2.setCurrentUser(currentUser);
                    controller.setMode(m2);
                    break;
                case Precision:
                    Mode m3 = new Mode(Modes.Precision, c, context);
                    m3.setCurrentUser(currentUser);
                    controller.setMode(m3);
                    break;
            }
            stage = new Stage();
